...
- **The other `app_*.arb` files are never hand-edited.** `app_ar`, `app_de`, `app_es`, `app_kk`,
  `app_pl`, `app_pt`, `app_uk`, `app_zh` and `app_zh_Hant` are written by
  `scripts/translations.ps1` from what the translation page on lumitlab.com sends back, so a
  hand edit is overwritten by the next run. A new English key leaves the other languages short, which is expected and falls
