...
Lumit's interface. Terminology follows [01-GLOSSARY.md](01-GLOSSARY.md) exactly. All colour,
type, spacing, and iconography specifics are deferred to [15-DESIGN.md](15-DESIGN.md) — this
document says *what exists and how it behaves*, never what it looks like.
...
> **Shell shape.** The shell is a tiling dock. Panels stacked together form a
...
> anywhere empty in it. It carries **no drag grip of its own** (the corner square of
...
Easing panel is in no other arrangement and **Nodes**' because the
Graph and Node panels are in no other.

**No shipped arrangement carries the Hierarchy panel** — not the default, and not
...
**Nor the Debug panel.** It fronted the right column once, and sat behind Effects & presets
after that panel took the column; a developer's readout is not what a workspace opens on.
It keeps its own row in the Window menu.

**Edit, Effects, Nodes and Colour give the Timeline the same height** — 0.32 of
...
- **Edit** (default): Project panel left, fronted, with Effect Controls tabbed
  ...
  Scopes tabbed behind (the panel used to be a fourth tab on the *left*, buried
  behind Project, with Debug fronting the right column instead; Debug has since left the
  arrangement altogether); Timeline across the full bottom at roughly one-third window
...
- **Effects**: Effect Controls promoted to its own left column beside the Project panel;
  ...
  more of the column than the panel can spare. Seeing the picture *at* one effect rather than at
  the end of the stack is not a panel of this preset or any other: it is the Viewer's own
  **"at effect" chip** (§2.2), which appears over the picture whenever an effect is
...
- **Nodes**: the graph as the main surface. The Graph panel takes the
  ...
  the small Viewer's own chip, so this preset needs no second viewport for it. It
...
**The snapping switch is here.** One was here, and
...
**Tool options.** The armed tool's own settings, where After Effects puts them, and
empty for the tools that draw nothing:

| Armed tool | Options |
|---|---|
| Type | **Fill** swatch, **size** in pixels |
| Brush, Clone stamp, Eraser | **Fill** swatch, brush **size**, **hardness**, **opacity** |
| Shape, Pen | **Fill** swatch, **Stroke** swatch, **stroke width** in pixels — all live |
| The four Puppet pins | **Density** and **Expansion** of the mesh, in pixels — no fill: a pin lays no colour down |
| Roto brush, Refine edge | **Size** in source pixels — no fill and no hardness: a scribble lays no colour down and has no edge to soften, it says *this is the subject* over the pixels it covers |
...
(a width of zero draws no outline).
...
- Every tool button MUST carry a tooltip naming the tool and its current chord (§14). A tool
  ...
  of the application.
- **A tool whose behaviour is not built MUST NOT be armable.** Its button and its
...
- **A submenu MUST survive the diagonal to itself — the safe triangle.** A flyout
...
- A row MUST show the chord the keymap currently binds to its action, and MUST take it from the
  keymap rather than carrying a chord of its own (§15).
...
1. **Magnification** dropdown: Fit, Fit up to 100%, then 25 / 33.3 / 50 / 100 / 200 / 400 /
   ...
   **The Zoom tool** (§1.7): clicking zooms in about the point clicked, `Alt`-clicking zooms
   ...
   pointer is **drawn** (§2.3.3): Windows ships no magnifier, and Flutter's name for
   one silently becomes the ordinary arrow there.
   **Magnification MUST NOT change the resolution rendered.** The scale the engine is
   ...
   **The transparency board MUST cost the panel, not the picture**: bounded by the
...
2. **Preview resolution** dropdown: Full / Half / Third / Quarter / Auto (glossary §5).
   ...
   the project, and a comp that has never been given one previews at **Full**.
   Preview resolution MUST never affect export.
   **Its rows are option rows**: picking one runs it, moves the tick and leaves the
...
3. **Channel view**: RGB / Red / Green / Blue / Alpha (alpha as greyscale matte). One
   **bare mark** on the bar, opening a menu that lists the names in full. The closed face
   is a **coloured circle for the view in force** (§5's one glyph with colour of its
   ...
   a border round a colour is a box round a colour.
4. **Transparency grid** toggle (checkerboard behind transparent pixels instead of the comp
   background colour). An icon — the checkerboard itself — rather than the word.
5. **Wireframe/overlay menu**: layer wireframes, motion paths, mask paths, gizmo visibility,
   ...
   **Built so far**: it is the same menu as item 6 — the bar has one overlay mark
   — and it carries the layer-controls switch, which turns the wireframes, handles
...
6. **Guides menu**: rulers (`Ctrl+R`), guides (drag out of rulers; lock/clear), grid,
   ...
   **Built**: the menu itself, one mark beside the transparency
   ...
   **Snapping** is the toolbar magnet (§4.5, §1.7): with it on, a layer dragged on the
   ...
   per-project session**: a comp reopens with the
   scaffolding it was left with, no tick makes the project dirty, and Ctrl+Z never undoes
   one.
7. **Region of interest** (landed): drag a rectangle; the engine renders only that
   region for preview. MUST be clearable in one click and MUST never affect export. Armed
   from the view menu of items 5–6, swept on the picture, and outlined whenever it is in force. It is a
...
9. **Degradation indicator**: users MUST be able to tell a degraded frame from a final one
   at a glance. **Stated by the bar's reading rather than by a badge**: the
...
10. **Background colour**: per-comp background (project state), plus quick black / grey /
    custom. It is a **row in the view menu** carrying a swatch of the colour it
...
11. **Current time** readout in the comp's timecode; click to type a time. A time outside
    the composition lands on the nearest end rather than being refused.
12. **Exposure**: the number alone on the bar — 10px mono, no inset under it and
    no aperture beside it — scrubbing on drag and taking a typed number, reading
    signed stops to one decimal: `+0.0`, `+1.4`, `-2.3`.
    The number means what the Exposure effect's does: the same `2^stops` gain in
    scene-linear, so the two agree. **Preview only; it MUST NOT change the
    export.**
    A **reset mark** stands to the left of the number and puts it back to `+0.0`. It is
    there only while the exposure is not zero: the drawing puts none beside a
    resting exposure, and a mark that is always there says a control is engaged when it
    is not.
13. **Tone mapping** toggle: an icon, no menu. A fixed highlight rolloff — the
    ...
    It is a **row in the colour-pipeline menu**, inside the display transform it is
...
14. **Snapshots**: **a pair of marks**
    ...
    and not the whole of it — never more pixels than the panel has, and every
...
**Nothing on the bar may move as the picture changes.** Every part of it whose
...
### 2.3 Layer controls: the wireframe and the transform gizmo
...
- The **wireframe** is the box itself: the layer's own content rectangle put through its
  ...
  **Text measures its own line**: the point size tall and the engine's own width
...
- **A layer switched off is not on the picture**: it gets no wireframe, no hover
...
- **Dragging** a layer's body moves it; dragging one that is not selected selects it first,
  ...
  **A press inside something already selected takes that**, even where a higher layer overlaps
  the same spot; only a plain click still takes the topmost, which is how a layer
...
- **One gesture is one undo step.** A drag writes Position x and y, and a scale writes
...
- **Scale may be negative.** A handle dragged past the anchor turns the layer over,
  ...
  turn follows.
- The gizmo's **centre handle is the anchor point**, and dragging it pans behind —
  the pivot moves, the picture does not, and the **mark moves as it is dragged** rather than on
  release — with the same `Shift` axis lock and `Ctrl`/`Cmd`
...
- The **Rotation tool** turns the **selection** — every selected layer, each about
...
- The **Anchor point tool** (After Effects' Pan Behind) drags a layer's anchor while
  ...
  **The pivot goes where the pointer is**: a click places it there and a drag keeps it
  ...
  arms, centred on the point the pivot will land at. It MUST NOT carry an arrow or any
...
- The Rotation tool's **pointer is a curved arrow**, drawn rather than a system cursor
  ...
  **It settles on eight positions and nothing between them** — the layer's four edges
...
- **A preview in flight MUST be drawn in flight.** The picture is previewed at the new
...
### 2.3.1 The shape tools and masks

- With a layer **selected**, a shape tool draws a **mask** on it. With **nothing** selected it
  makes a **shape layer** at the top of the composition, holding the art it drew, in the
...
- Each item's **animatable numbers get a row each** under it, as a mask's and a stroke's do:
  Trim start, Trim end and Trim offset. A row rather than another control on the item's
  own row, because a property without a row has nowhere to put the stopwatch that animates it.
- **Path** is the first of an item's rows: the shape itself, which has no number, so the
  ...
  under the playhead exactly as a mask's does — and the row selects itself when it does,
  so the key is planted on a row the author can see.
- **Combine** heads the rows of every item **after the first**: an Apart / Union /
...
- **Fill** and **Gradient** head an item's rows, on any item that has a fill: a colour
...
- **Offset path** comes before the trim's rows, because it applies first: one length in
...
- **Dash, Gap and Dash offset** appear under an item that has an **outline**, and only then:
  three dead rows on a fill-only shape would be three promises the item cannot keep.
...
- **Copies** appears under every item, and the repeater's other nine rows — Copy offset,
  ...
  Copies is the row that opens them, so there is no separate "add a repeater" gesture,
...
- **The Pen builds a path point by point**: a click places a corner; a click-and-drag
  ...
  **The edge to the pointer MUST be previewed as the curve it would be**, bent by the
  ...
  edge MUST run to where that vertex was placed and bend into it by its own incoming
  handle — the shape that will exist when the button comes up, drawn as it is aimed.
  **A click that would close the path MUST say so before it is made**: the first vertex
  and the pointer both take a ring, so "how close do I need to be" is answered on the picture
  rather than by trying.
  **`Ctrl+Z` takes back one point while a path is being built**, and returns to the
...
- **A mask's points can be edited with the Selection tool** while the layer controls are shown.
  Each vertex is drawn as a small square, filled when it is selected. A click takes
...
- Masks appear in the layer's Timeline twirl-down under a **Masks** heading — above Effects,
  ...
  same header a layer's blend mode sits under — and its context menu renames and
  deletes it.
- **A mask's values are property rows, and every one of them keyframes.** Under the
  ...
  without a curve.
- **A feather can be a width per point**, and then a **Point *n* feather** row joins
  ...
  Viewer does not evaluate one while it paints.
- **A mask (and a shape item) is renamed in place.** A shape drawn with a tool is named
  ...
  rename (`Enter` opens that, because a double-click on a *layer* name opens the
...
- **A mask row is an ordinary property row.** Clicking its name selects it, with the
...
still. Not built: bezier **handles** on any path, mask or shape (the
...
- The document MUST be written **once**, when the edit ends — one typing session, one undo
  step — with the picture kept in step meanwhile by the text preview path.
  Ending an edit means `Enter`, `Escape`, clicking elsewhere, or putting the tool down.
- **Making the layer is one op, and finishing the edit is one more.** So the first
...
- **`Ctrl+Z` while typing MUST end the edit and then undo.** The text field swallows
...
- **The box grows with the words**: what is being typed is what the wireframe measures,
...
however slowly the application repaints (an app-drawn pointer repaints at the
...
**Windows ships neither a grab nor a magnifier.** Flutter accepts `grab`, `grabbing`,
...
A `MouseRegion` stops reporting a hovering pointer the moment a button goes down, so a
...
| Razor (Timeline) | The scissors icon, with the cut line doing the aiming (§4.4) |
| Razor (Viewer) | The ordinary arrow: it cuts in the Timeline, and a precise pointer here promised a gesture the Viewer does not have |
| Hand | A drawn **open hand**, closing while it pans |
| Zoom | A drawn **magnifier**, its sign following `Alt` |
...
- The cloud is asked for **once per frame change**, never per rebuild — the Levels histogram's
  rule, and the bridge-call budget is the gate.
...
- A **solve-linked Camera layer** wears its state as a calm badge in the Transform heading of
  ...
  editable: what they hold while linked is a **correction** added to the solved motion,
...
- **A stroke is stored in source raster pixels**, carried there through the whole
...
- **A first scribble on a layer with no Roto brush just works**: the brush is added with the
  stroke riding inside it, so the whole gesture is still one op and one undo step. **Release
  solves the scribbled frame's own matte immediately** — the same job Propagate runs, stopped
  at that frame, with the solving status on the effect's card while the second or so passes —
  and Propagate stays the road to the rest of the shot.
...
- The Roto brush's **View** row chooses the picture: *Result* and *Matte* are drawn by the
  ...
  never per rebuild, and only in that view.
- **The effect's own card** carries Propagate and Cancel (ordinary Action rows), the value rows,
  a **span bar** in §6's two-weight form — the frames the matte covers in the accent, the
...
results; stale frames MUST never be presented as current without the degradation
indicator lit.

**Preview progress.** A frame the user is waiting on — a scrub, a
...
- A folder tree with drag-to-reorganise: dropping rows on a folder row files them there,
  ...
  (workspace state).
- **Under the redesign (built to the mockup 2026-08-24)**: the panel has a bottom
  ...
  *folder* a footage item points into, hushed to `text_disabled`, empty when the project
  ...
  right or the row menu's chip strip — both raise the same choice — and the
  **swatch filter inside the search well** — one square that opens the eight-colour
  picker — narrows the tree to one colour — on the colour a row is *wearing*: its own
  tag, else a folder's handed down, else the kind's default tint (azure for picture
  footage, indigo for sound, amber for solids), so a colour
...
**An image sequence imports as one row** ([03-DATA-MODEL.md](03-DATA-MODEL.md) §3.1).
...
- **Colour space tag**: the footage's colour space for conversion into the working space.
  ...
  on the item's context menu (docs/impl/ocio.md §6.5) — the built-in interpretation,
...
- **Sequence frame rate**: for an image sequence this is not an *override* — stills
...
- Each footage item shows a **proxy badge** when a proxy exists (glossary §5): states are
  ...
  **Shipped:** the row wears a colourless `proxy` badge while its own
  *use proxy* tick is on, and the row menu carries the four commands on footage
  ...
  opinion about a stale proxy (the renderer falls back silently).
- **Missing footage** shows a distinct badge and renders as a placeholder slate in comps
  ...
  **Shipped:** `MediaStatus::Missing` is its own state, distinct from an unreadable file —
  ...
  so the mistake cannot hide in a delivered file. Relinking one item batch-relinks
  every other missing item whose name is found beside the chosen file, in a single undo step,
  each rebased relative and re-fingerprinted. **Find missing footage** is a toggle
...
[impl/timeline-interaction.md](impl/timeline-interaction.md)**: the
...
**markers ribbon** and the **work area bar** covering the whole of it, and — on
...
- **Work area**: `B` and `N` set start/end at the playhead; drag the ends; double-click the
  ...
  **one band in `animated`** ([15-DESIGN.md](15-DESIGN.md) §12A.1): the ruler's **second
  ...
  through the lanes, behind the bars and the keys. Its **two drag handles keep the ruler's
  whole height** and are **drawn**: a small vertical rounded tab in the band's
  ...
  **double-click that clears it** is the
  ...
  row that had a layer in it.
  A comp that has never had one set **reads as the whole comp** — the engine's
...
- **Cache bar**: a thin stripe showing cached frames per tier — VRAM, RAM, and disk caches
  ...
  The bar MUST update live as background rendering fills the cache.

**Shipped header arrangement.** The comp tabs span the panel; each tab is an
...
session. **Fronting a comp puts you back where you were in it**: the playhead
...
fully zoomed out. Like the Viewer's exposure, the preview resolution and
the region of interest, this is session state — it rides in the session blob and
so in the `.lum`'s `ui_state`, never in an op, so a scrub never lands on the undo
stack or makes the project dirty. **A tab that is not the fronted one outlines under the pointer**: the value well's own hover edge, one pixel of `hairline_strong`, drawn over the tab rather than inside it so nothing on the strip moves as the pointer travels. The fronted tab is already marked by its seated surface and adds nothing under the pointer. Right-clicking a tab opens **Composition settings…** for that comp, the same
...
frame readout `f72` — both in **fixed-width slots** and both **click-to-type**:
...
bar tucked under it. The ruler is **double height and reads as one band**:
...
at one tick per frame. **Both of the outline's chrome rows grew under Regular** -
...
own: an **upward triangle** standing on the cache bar at the floor of the ruler,
...
**Layer markers** draw on the layer's own bar, in the same flags, and travel with
...
**Opening an item.** A double-click — or a second click on a row that is
...
Items are therefore renamed with **`Enter` on the selection** (§15) or from the row
...
**One rename gesture, everywhere.** No surface renames on a double-click or on a
...
**Opening a layer.** Double-clicking a layer in the Timeline outline opens it the
...
the moment is mapped through the layer's start offset and its Retime, so a
...
**A modal window's keys are its own.** Panel commands stand down while a dialogue is
open. A dialogue's default action takes focus when the window opens, is drawn with the accent
edge, and `Enter` presses it.

**Escape dismisses.** Every modal MUST answer Escape by dismissing — the same
...
**Escape cancels an inline editor too, and writes nothing.** An inline rename or an
...
other exit commits (§4.3) — Enter, clicking away, losing focus — so Escape is the one
...
**Every window, not just one.** That rule is now the shape of *all* of them: each
...
**Tab order is reading order.** Focus moves left to right, then top to bottom, by
...
3. **Switches** (glossary §2): visible, audible, solo, lock, shy, quality (draft/full),
   ...
   into a comp that has a solo up arrives soloed** — a solid, a title, a shape, a
...
5. **Matte** dropdown + pick-whip: choose any layer in the comp as this layer's matte
   ...
   never a camera, an audio-only clip, or the layer itself. Layer-valued **effect
...
**Shipped arrangement:** the columns sit in
...
· collapse (with 3D and motion blur swapped by the owner's 2026-08-31 desktop testing: fx
leads the column, collapse has its own cell at the end rather than sharing the earlier
flow-or-collapse slot, and flow stands immediately left of it);
...
group resizes it** — every other group keeps its width, so the outline grows or
...
group** (15-DESIGN §12A.1): a kicker naming each of the A/V, switches and
...
from Lumit's own icon set** wherever the set has the mark — visible/hidden,
...
footage and **collapse now has a cell of its own**, drawn on a Precomp — so a
...
preserve-underlying-transparency still await their backing machinery;
...
lights** entry, which is where that setting lives now that it has no cell in the
Modes column. The cell it freed is the **adjustment switch**: on, the
...
way, and applied to the whole selection like the switches beside it — **as one
edit**: a switch click on a multi-selection commits one batch of per-layer ops, so
...
**The render-time column ([13-PERFORMANCE-RULES.md](13-PERFORMANCE-RULES.md) §7.1)**
...
and composited again, measured, when the worker is next idle: the cache bar's green
...
members as an implicit per-frame precompose (docs/impl/group-effects.md), which is
...
- The header's right-click menu offers **Rename group**, **Ungroup**, and **Pre-compose
  ...
  render-level collapse a group deliberately is not. The pre-compose also
  carries the header's effect stack onto the new Precomp layer and retires the emptied
  band, as one undo step.
- **Effects on the header**: while the header's stack is non-empty the identity
...
- **A section heading is selected by clicking its name, and twirled by the same click.**
  A *modified* click — `Ctrl` to toggle, `Shift` to extend the run — only selects, so
...
- Each animatable property lane shows: stopwatch (keyframing on/off), value with
  **scrub-drag** and click-to-type numeric entry, expression toggle, and its keyframes as
  ...
  the graph editor.
- **There is no auto-key**: the stopwatch is the whole model. A property animates
...
- `U` reveals animated properties of selected layers; `UU` reveals all modified properties;
  ...
  **Shipped.** The taps are counted in the panel — a multi-tap is a gesture, like a
...
- **Animation ▸ Reveal properties with keyframes / with animation / all modified
  ...
  times. *With keyframes* is `U`'s own answer, the rows carrying diamonds; *with
  animation* adds the rows that move without them — an expression, or a wire from the node
  graph; *all modified* adds everything a fresh layer would not carry. They open
  ...
  composition when nothing is selected, exactly as `U` does. **Shipped.**

  **The first tap stops at the keys.** `U` opens down to the keyframed *rows* —
  ...
  **Shipped (partial):** the caret on each layer row opens onto the **section headings**, each
  ...
  - **Retime**, only on a layer that has been given one with **Ctrl+Alt+T** or
    Composition ▸ Enable Retime: a single
    ...
    flattened to one constant removes it too**: turning the stopwatch off, or deleting
  ...
  summoned by the gesture and gone on release, like every other transient hint (the
  ...
  group 3, aligned to both its edges, so the numbers stack into one column wherever
  the groups are dragged. **An animated value stays editable**: the field shows the
  ...
  panel that draws a property row**: where a driver is wired to it, the
  ...
  committed on release. **Clicking a property's name selects it**: the
  ...
  closing a fold drops the selection inside it, selecting a layer clears the
  ...
  selected — on every layer in the comp. Mask rows are in that same selection, and
...
- **Razor**: with the razor tool (`C`) click a layer to cut it **at the time under
  ...
  the pointer only says which tool is in hand: a drawn
  ...
  **A cut only keys a layer that has actually been retimed**: switching Retime on
...
- **The sequence view**: double-clicking a Sequence layer — its outline name (where
  ...
  pointer/playhead, and a small **speed-envelope strip** (speed lens only) sits under
  the clips, its points travelling with their clip. Clips may be **reordered and repeated**
  (they need not follow source order). The layer's bar always spans first clip start →
...
read, and it was taken off (§1.7) on the understanding that it belonged there once
there was snapping outside the Timeline for it to govern. **There is now, and it is back**:
the toolbar's **magnet** governs snapping on the *picture* — a dragged layer
...
**Shipped:** the **magnet** in the lane bottom bar, on by default. With it
...
there is nothing near, which was the original and much narrower behaviour. Beat-marker
...
- **The bottom bar's zoom is a slider** between a small landscape glyph and a large one — the
  ...
  under 16px without crunching. Its left end is the whole composition;
...
- **A slider zoom holds the playhead still; `Ctrl+wheel` holds the frame under the pointer.**
  The slider has no pointer to zoom about, and the playhead is where the work is —
...
- **A dragged slider MUST choose its anchor once, at the start of the gesture, and hold it
  to the end.** Re-measuring per update reads the scroll offset *before* layout has
...
- **The scroll correction that holds the anchor MUST happen inside layout**: the
  ...
  that disagree.
- **A dragged zoom control MUST NOT animate.** The flight fills the gap between zooms
...
- **A trackpad's two-finger scroll MUST scroll the panel.** It arrives as a pan
...
- **The outline and the lanes MUST scroll exactly as far as each other**: they are
...
- Dragging in the ruler scrubs the playhead. Scrubbing previews video always; holding
  `Ctrl` while scrubbing also scrubs audio. **Scrubbing while playing stops playback** and
  the playhead stays where the drag left it: the engine hands back a frame
...
- The playhead MUST stay visible during playback via edge-follow scrolling (page-flip or
  ...
  anything.
- **The time navigator**: a slim strip across the top of the panel, above the ruler,
  ...
  *view*, and drawing the document too would be a second ruler at the wrong scale two pixels
  ...
  outline's timecode row is **taller by exactly the strip's band** (the owner's
...
**Shipped:** the outline and lanes scroll vertically as one table — one
...
Still to build: the *smooth* alternative and the setting that chooses between the
two, and `Shift+=`.
...
- Layer drag moves in time; vertical drag reorders the stack. `[`/`]` move the selected
  ...
  **Shipped:** dragging a layer's **bar** moves it in time, and dragging a layer's
  ...
  **Dragging one selected bar drags the whole selection.** A move drag that starts
  ...
  **What the lock means.** A locked layer refuses every edit to what it *is* — its
  ...
  **A trimmed layer shows its source's reach.** A source-backed layer that is not
  ...
  is exactly the material trimmed away. **A hairline and nothing inside it** (15-DESIGN
  §12A.1): a fill behind the bar read as a second, dimmer object rather than as
  ...
  **The bar fills desaturated behind a solid leading edge (15-DESIGN §12A.1).**
  ...
  **A shut layer shows what is keyed inside it.** Its own row draws every keyframe
  ...
  **Switching Retime off re-hangs the layer on its source.** A retimed layer may be
  ...
  **Both halves move.** While the drag is in flight the stack shows where the drop
...
outline is the Layers outline, identical**: a property's curves are on the pane
...
curve without ever scrolling the layer list. The ordinary layers view keeps the single
...
- **Value graph**: value against time, editable bezier tangents per keyframe side, following
  the AE-compatible keyframe maths (per-side speed in units/second, influence
...
- **Acceleration graph**: the second derivative against time — the
...
- **Lens switch**: value / speed / acceleration are selected by glyph buttons in the
  **bottom-right of the graph editor**, beside the ease-preset footer (§5.3).

### 5.2 Retime's two lenses

A **retimed layer** exposes its Retime as a channel in the graph editor's left column, beside
the transform properties. A Sequence layer's clips are retimed in the sequence view
instead (§4.4).

- The **value lens** plots source position against layer time — the ordinary property graph.
  It reads in seconds for now; a **frame timecode** readout (`HH:MM:SS:FF` in the
...
- The **speed lens** plots speed percentage against time, and its shape follows the Vegas
  preference: **off**, the ordinary two-sided derivative view every property has;
  **on**, the **envelope** — one point per key, whose height *is* the speed, with a
  default vertical range of 100% down to −25% that grows to fit the curve. Either way it is
  drawn **in the graph pane, never overlaid on the clip** in the Timeline; the bar
  shows only read-only indication.
- **Default lens**: the Vegas preference chooses which lens a Retime channel opens to — on,
  speed; off, value.
- The store is the one Retime property; switching lenses never converts or degrades
  data. Reverse is legal in both lenses and both modes.
...
- **Box-select** keys by drag; add with `Shift`. A transform box around a multi-selection
  ...
  with `Shift` rounding what the scale lands on (not corner drags, and `Ctrl` does not
  taper: corners stand on the selection's own extreme keys, and `Ctrl` already suspends
  the magnet).
...
- **Tangent modes** — Auto / Clamp / Free, a run of three on the key-command strip, stored per
  key side. An automatic side takes its speed from the key's neighbours on every
...
an **Easing…** button beside the three one-click eases opens a unit box — travel
...
a graph the user is not looking at. **The box is the Easing panel** (§5.4): the
...
in a popup* puts the same editor in a floating box over the footer instead. **A drag in the graph previews as it goes**:
...
**The selection transform box and numeric entry**: two or more selected keys in
...
- **The shipped shapes** are five house basics — Easy ease, Slow start, Slow finish,
  ...
  basics' "slow start" naming without a collision.
...
- **Parameter widgets** by type: sliders with scrub-drag and click-to-type; **colour
  ...
  **track-and-thumb sliders** for a closed range; curve editors
...
- **Preset save/load**: save the selected effect (or whole stack) as a preset; presets
  ...
  **Shipped: drag-to-reorder** — dragging an effect's heading onto another's moves
  ...
  **Shipped: the panel's layout.** The panel is **one list, not a stack of cards** — the same
  ...
  adjustment to it. **Reordering is a right-click on the heading** — move up, move
  ...
  **An effect's name selects it**, taking the selection fill across the heading bar —
  ...
  **An effect can be given its own name.** `Enter` on the selected effect turns its
  ...
  **A parameter can be a button.** An `Action` row carries no value at all — no
  ...
  **A finished answer that covers part of its clip draws the part it covers.** Where
  ...
  **Two parameters can share one row.** Two conventions fold, and both exist because the
  ...
  [08-EFFECTS.md](08-EFFECTS.md) §2.6, labelled "Matte" and "Invert", which every
...
### 6.1 The colour picker and the dropper
...
picker**: a project-wide swatch row is the picker's own, never a swatch strip in
the toolbar.
...
**A pick is a drag, not a click.** Pressing on the picture writes nothing. It starts a
...
frame transport that was taken out (a 1080p frame is 8 MiB and 8.8 ms in the codec); it is the
...
### 6.2 Value boxes and text fields
...
- **User presets**: a **Presets** group at the top of the tree lists the `.lumfx`
  ...
  the current selection**: the highlighted effects with their values as set, and — when
...
- **v1**: scopes are computed on the CPU from the composited
...
The v1 sync toolkit; the Composer workspace is future work specified in
...
- **Waveforms in the Timeline**: every audible layer MAY show its waveform inside its row
  ...
  **Shipped:** the Audio group (Volume + Waveform twirl) in the layer
  outline; the lane draws the layer's own peaks through its live offset each paint. The
  peaks are **mip-mapped and window-fetched**: the lane asks the engine for the
  ...
  same way: its buckets are taken in the layer's clock and mapped through its Retime,
  so a slowed passage is drawn wide because it is wide. The lane is drawn across **both** its
  own row and the empty one its Waveform twirl sits in, so a centred wave sits on the
  ...
  **multiwave** stack (bass / middle / treble) by default, drawn over one another around one
  centre line rather than in separate lanes; Settings ▸ Interface ▸ Editing ▸
...
- **Volume keyframes**: each audio-capable layer has a Volume property (dB) with normal
  ...
  selected layer's Volume and Pan (pan shipped)
  with their stopwatches, fade in/out wells with the three curve chips, and the
  two graph-template buttons — *Drive with audio…* (Audio level → Remap → Smooth onto a
  picked parameter) and *Lower behind…* (the ducking move in plain words: the
  inverted chain onto this layer's Volume socket).
- **Level meters**: the Audio panel's Levels section draws the output's stereo bars with
  ...
  draws a strip per sounding row — name in the layer's label colour, pan
...
- **When done**: two ticks, not a list — *make a noise* and *open folder* — because
...
- **The dialogue is one scrolling page**, and its tab strip — Output / Time /
...
- **A control the chosen format cannot honour is disabled, never hidden and never live**
  (the format's own capabilities decide): an `.mp4` has no alpha channel and no sixteenth bit,
  an image sequence has no sound and no bitrate, a `.wav` has no picture at all. The same face
  carries the one row no *subsystem* backs — proxies — with a short reason on hover; *render
  guide layers* is a live tick the day the seam carries it (the subsystem is built). Whatever the engine would refuse is said **in the footer, before
...
The **Hierarchy** panel shows the active composition as an indented, foldable tree:
...
## 12.2 The FX console (`Ctrl+Space`)
...
raised, and the scrim under it, are gone by the owner's ruling: the console is **one search
...
**While the console is open, the keyboard is the console's.** The search field MUST
...
- The **search row**: the magnifier, the query, and a kicker at the right naming the key
  ...
  none. The snapshot button sits at the row's end.
- The **category strip**: `All`, then every grouping the results carry (the browse
  groupings, drivers filed under Controls). Choosing one narrows the list to
...
- The **list**, open from the first frame — the list is the offer, and typing narrows it.
  ...
  preview swatch. `Enter` applies the highlighted match to **every** selected layer; a
  composition fronts. The arrow keys move the highlight. `Esc` retreats **one
  step at a time**: clear the text, then close.
- The **foot**: one quiet sentence saying what choosing a row will do ("Enter applies to
  the selected layers"; the canvas's own words). Drawn only where the caller
  has something to say.

**The snapshot button** writes the frame on screen to a PNG. It MUST be a one-frame
image-sequence export (§11's `png` codec) rather than a second still-writer: same
...
### 13.1 First-run setup (v1 ships minimal)
...
The **v1 build** ships the minimal form of this screen: two plain choices,
**AE-style** and **Vegas-style**, where Vegas ticks two settings (Retime opens to
speed; video arrives as a Sequence layer) and AE ticks neither. Along the bottom sits one
tick, **on by default**, for automatic update checks — the same setting as
...
- **The welcome screen** (superseding the card above once the redesigned
  shell lands): the launch window carries **exactly two** start cards — **New project** and
  **Open** — with **Manual** and **What's new** as outlined buttons, and no "free and
  ...
  last saved, its name, its path, and when it was last opened here (no format
  ...
  **Clear** that empties the list and a **×** on each row that forgets just that one.
  ...
  A footer carries the **product** version — `Lumit 0.2.0`, not the boot line's crate
  name — and the two links. **New project asks nothing**: it hands the window to the
  editor on the empty project already loaded, and where the file goes is the first save's
  question. **Escape closes the screen** with nothing open, and
...
- **Tooltips policy**: every icon control has a tooltip with its name and current shortcut,
  ...
  rather than narrating the click. **There is no long form and no exception list**:
...
### 13.3 The composition settings dialogue
...
([15-DESIGN.md](15-DESIGN.md) §12A.4): a kicker title strip, rows of 30 in a 110px
...
- **Duration**: reads and edits as `HH:MM:SS:FF` timecode at the frame rate above (the same
  ...
  under layers that had not moved.
...
- **Motion blur**: the master shutter's **angle** in degrees and the number of sub-frame
  **samples** a blurred layer is drawn at. The shutter's phase and the master on/off
...
**The playhead keeps its moment, not its number.** A frame count means nothing without
...
any other. The three answers are remembered in the workspace across launches; the
...
### 13.5 The Project settings window
...
- **Anti-aliasing** — the number of coverage samples per pixel the composite is drawn with
  ...
  preview–export parity rule. Where the graphics card cannot manage the count asked for, a
  second row states what is being used instead, in the calm voice; the project keeps the
  value its author chose and nothing is rewritten behind the user's back.
...
cache's *Applies to* row stays in Settings ▸ Performance: choosing between the two
scopes is that control's whole job, so it is the one that stands in both.

### 13.6 Floating windows
...
- **First click selects, always**: selection lands immediately on the first click,
...
- **No punishment UI**: errors (missing footage, failed export, expression errors) are
  ...
  never a modal.
- **Voice**: UI copy is en-GB, sentence case, no exclamation marks.

### 14.1 The Escape ladder
...
Escape on its own focus node, which runs after every keyboard handler, so it is reached
...
- **Performance**: RAM budget for Lumit (default 60% of system, slider + absolute),
  VRAM budget (default 70%), CUDA acceleration on/off (it is only ever an
...
- **Preview**: default preview mode (Cached/Realtime), Realtime tier bounds,
  adaptive-degradation aggressiveness, audio scrubbing on/off.
- **Colour**: working-space defaults for new comps, display transform selection,
...
- **Rendering** — *not here at all*: it is the project's, not this machine's, so it lives in
  the **Project settings** window instead (§13.5).
- **Updates**, under General: *Automatic updates* — look for a new version at
...
- **Language**, under Interface: which language the interface is written in.
...
Export… — the **Shape**: Studio, Desk or Lantern, under Desk the **Rooms drawn** pair,
Grey room and Graphite, and under Lantern the **Room**: the style's choice, Day or Night
(15-DESIGN §12),
...
Deck (§2.2), the **Toolbar position**: Top or Left (§1.7), the **Range sliders** switch
(§6), the first two the style's choice until a person picks, the switch on by default, a
**Command box** switch, on by default, for the search well at the right of the top line
...
**The disk tier's controls have landed**, as a third section on the same page: its
...
dropdown. An **Applies to** row beside it chooses the scope: *Everything*, kept in the settings
...
how often a spare copy is written, in minutes, and how many copies are kept, with
zero minutes meaning off. **Export** is its own page too, and is the last of the
...
already substitutes — `{comp}`, `{preset}`, `{date}`, blank giving each preset's
...
Inside the **Timeline** `L` reveals a layer's Audio instead — the panel where you
...
**Shadowing is not a clash.** A binding scoped to a panel takes a chord over from an
...
**Shipped.** Settings → Keymap is a table, grouped by the context a binding is live
...
(reported as a shadow note rather than a banner). One row, one chord: no
...
in the Project panel and in Effect controls, and the Panels context cycles the focus
ring and focuses a panel's search field. Stepping a frame has a second
chord alongside `Page Down`/`Page Up` — `Ctrl`+arrow; the **bare** arrows do nothing
...
| Global | `Ctrl+→` / `Ctrl+←` | Next / previous frame (`Cmd` on macOS) |
...
| Global | `*` (numpad or `Shift+8`) / `Shift+M` | Add marker at playhead. `M` keeps Reveal Masks, so the letter form takes Shift |
| Global | `Shift+0…9` | Set numbered marker at playhead — pressing it again *moves* that marker, and it replaces whatever is on that frame |
| Global | `0…9` | Go to that numbered marker; nothing happens until one has been set |
...
| Global | `Ctrl+X` / `Ctrl+C` / `Ctrl+V` | Cut / copy / paste the selection — picked keyframes, else the selected property rows (their keys, or their plain value where they have none), else the picked effects, else the layer |
...
| Timeline | `U` / `UU` | Reveal the keyed rows / every modified property |
| Timeline | `L` / `LL` / `LLL` | Reveal Audio / and its waveform / shut again (`Shift+L` does the same). Inside the Timeline this takes `L` from the shuttle transport, which keeps it everywhere else |
...
| Project panel | `Enter` | Rename the selected item |
| Effect controls | `Enter` | Rename the selected effect (the heading, not a parameter row) |
...
macOS development builds map `Ctrl` to `Cmd`.
...
specified in [15-DESIGN.md](15-DESIGN.md). This document intentionally contains no
