...
How to stand up `lumit-gpu` so everything above it stays simple. The traps here are
...
- The Viewer's checkerboard, guides, gizmos are egui painting on top; the frame itself and
  the display transform (§2) happen in Lumit's blit **before** egui sees it, so egui's
...
- The neutral-surround rule ([15-DESIGN.md](../15-DESIGN.md)) is enforced here: the Viewer
  panel background comes from the theme's `viewer_surround` token, drawn by Lumit, not a
  generic egui frame.
