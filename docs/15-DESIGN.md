...
household Aizome system; since the 2026-08-23 redesign it **stands on its own** — the
...
starting point are deliberately retained. But since the 2026-08-23 redesign **Lumit's type
and palette decisions stand on their own**: no household file is an upstream for Lumit's
fonts or colours, and a change to the household system implies nothing here.
...
- **Dark-first** (the half of the original ruling that still stands): dark is the native,
  first-built, first-polished theme, because a neutral dark surround preserves colour
  judgement against the Viewer.
...
- **Type discipline.** The faces changed (§7.1 — Hanken Grotesk and Geist Mono), but
...
- **KD-1 · Dark-first (DECIDED).** The household default was paper-light;
...
- **KD-2 · Hit-target compensation (DECIDED).** The household accessibility
...
the 2026-08-23 redesign the household `theme.css` is no longer an upstream either: Lumit's
palette is re-derived from nothing, and a household repaint implies no change wave here.
Person-identity colours have no meaning in a single-user pro tool; `sky` survives only as a
curve colour (`curve[0]`), and nothing becomes a second accent — the second stateful colour
that exists, `animated`, is not an accent and has a closed job list (§3.1).
...
The ramp's *structure* follows rerun.io's viewer (`re_ui`): a near-black canvas, panels one
small step above it, and floating surfaces a clear step above those. The hues stay Lumit's
own, and the values in this section are that system.

### 2.1 Surface ramp

Five surface levels on the rerun-inspired structure: the canvas sits near black,
...
- **Three greys at rest.** A panel in its resting state shows at most three surface
...
- **The surround of the Viewer image area MUST be `viewer_surround` — strictly neutral grey,
  ...
  Settings → Appearance → Viewer offers a switch that paints the surround in the
...
input wells are the §2.1 inset, not a raised fill. **Buttons are the one exception**:
...
|---|---|---|
| `accent` (spruce) | `#35785e` | THE accent, with a deliberately short job list: **the single filled button per surface, the playhead, and the active tab tick**. The selection tokens (§6.5) derive from it. Everything else in chrome is grey. It is the **one** value to retune — both stock schemes build their pair from `LumitTheme.defaultAccent` |
| `animated` | `#d8a24a` (placeholder, tunable) | "This is animated or in hand": **keyframe diamonds, stopwatch-on, selected keyframes, selected gizmo handles, the focused value field, the work-area band, and the selected node's border in the Graph panel** — a desaturated warm amber, quieter than `accent`. That list is closed: **if a further kind of use appears, it is wrong** |
| `accent_hover` (spruce-light) | `#478a70` | Hover/active shift of the accent — `accent` ±`0x12` a channel, the same step `with_accent` gives a user-picked one: lighter in Dark, and `#23664c` (darker) in Light |
...
**Editable values.** An editable value is **mono text in an inset well** (§2.1) at
...
passed by reference. Since the Flutter port it lives Dart-side as `LumitTheme`
...
pub struct Theme {
    ...
    pub animated: Color32,          // the closed job list (§3.1), nothing else
    pub success: Color32,
    pub warning: Color32,
    pub error: Color32,
    ...
    pub port: PortColours,          // wire/socket colour by data type (viz-family) —
...
**Custom themes.** Every colour listed here is editable by the user in Settings →
...
**Sharing a theme.** A theme is also a file: `.lumtheme`, an indented JSON document
...
`marker` grouping to be split out; the beat variant still waits) and `waveform`
(`WaveformColours`: `rest` plus the three multiwave bands). Not yet split into their own tokens, and derived ad-hoc from existing roles in
...
upstream. Since the redesign it is kept as history — the names still hold, but no household
file constrains their values, and new tokens (`animated`, everything in §6) need no household
counterpart:
...
| `clay` / `clay-deep` | `accent` / `accent_hover` | Mapped by **role**, not hue: the accent is spruce (clay came back for a day and spruce returned, taking the mark with it), and clay survives as one of the six Settings presets |
| `olive` | `success` | |
| `kraft` | `warning` | |
| `fig` | `error` | |
| `cloud` | `disabled` | |
...
**Lumit draws its own icon set** (superseding the earlier Iconoir pick). The set is drawn
...
- **Monochrome via `currentColor`**: a glyph takes the text colour of wherever it sits —
  ...
  a single circle for R, G or B, and a near-white one for alpha). It is painted
...
- Pixel honesty carries over: a 1.5px stroke is offset so it lands on pixel
  centres rather than straddling boundaries at 100% scaling. **The 16 is the drawing grid,
  not the display size** (superseding the earlier fixed 16 where a mockup renders
...
- **A glyph has to be readable as the thing it names**, and a mark that reads as something
  else is a defect however elegant it is. Three of the owed drawings were redrawn on
...
### 5.1 Chrome labels: Words / Icons / Icons everywhere
...
**Shape layer** and **Null layer** likewise borrow — their reserved values
...
- Keyframe markers at rest: `text_secondary` fills with `surface_1` outline. Selected:
  `animated` fill. Hovered: `animated` @ 40% halo. Interpolation is encoded by
  *shape* (diamond linear, square hold, circle bezier), never by colour alone. **In the
  ...
  The graph editor keeps the circle: a curve is what it draws, so the shape there
...
All three of Nebula's tiers ship (docs/06 §5.6). Every run draws as a 3px band at
the ruler's floor:
...
where each tier has its own bar. **Tiers differ in both brightness and fill height**:
...
Under the redesign the bar also carries the **resolution tier**, so a glance says
...
- **Overrun** (Retime requesting time beyond the media): the affected span of the
...
- **Comp markers (shipped)**: a `marker` token of its own — a plain grey, `#c4c4c4`
  ...
  *careful*, and the work area already has its own colour (`animated`, §12A.1). Under the
  redesign the flag is an **upward triangle**, 8 wide and 6 tall, standing on the
...
- **Clip waveforms (shipped)**: `waveform.rest` = `#5d8a96` (muted steel-cyan) filled
  ...
  when this grouping became real tokens. The **multiwave** stack adds three band
...
- **Selection**: `accent` 1px border + `accent` @ 16% fill on clips, layers, assets;
  ...
  `surface_0`. A bare hairline is findable only by hunting along the ruler, and at
...
**Two faces, both SIL OFL, both bundled: Hanken Grotesk** for UI text and
...
Lumit is a pro tool; the scale is tight (the 10–13px band stands) and **nothing in
...
- Row and chrome heights are **§12A.6's table**, which supersedes the 28px/24px rows
  written here before the redesign: an outline or lane row is 23 and a secondary row 19
  under Regular, 22 and 18 under Compact (the Timeline's own two chrome rows are
  24 and 23 under Regular — read the height off the table's two columns, never off
  this sentence). The
...
- Micro-motion (hover fills, panel tab underlines, drawer/menu entrances, drop-target
  ...
  overshoot on drop.
- **Animation level**: a three-tier in-app setting — **All** (this section's ≤150ms
...
- **AccessKit** (already in the stack) is wired from day one: every control exposes
...
- **Visible focus** everywhere, per §6.5. Every house control — button, checkbox, radio,
  ...
  (docs/07 §4.2).
- **A pointer travelling to a submenu is not hovering what it passes over.** Menus
...
- British English, sentence case, calm, no exclamation marks, no emoji. UI strings go through
  the i18n table — `flutter_ui/lib/l10n/app_en.arb`, translated through the site's
  translation page.
  British English is the source and stays the source; there is no en-US.
- **A tooltip is a name, not a lesson**: **one or two words, never more** (tightening
  [07-UI-SPEC.md](07-UI-SPEC.md) §13.2's "under five"). Under the Icons chrome
...
than a port. **The accent is the exception**: spruce clears the same contrast floor on white
(5.3:1) that the clay it replaced only reached after darkening, so Light and Dark share one
`accent` and differ in the hover alone — both stock pairs are exactly
`with_accent(default_accent)`, with no hand-tuned value in either.
...
The 2026-08-23 redesign fixed the resting layout of the main surfaces. A set of
...
- **Modes are Layers and Graph.** Keys —
  ...
  bar** (placing the redesign's Animated filter and superseding its `(U)`): the same kind of
  ...
  which came to the Layers bottom bar when Keys went and to the outline's foot
  when the lane bar was pared back.
- **Composition tabs run the full width of the panel header**, between the panel's own
  ...
  **The frame count says how far through**: the frame in hand, then the comp's
  whole length after it — `F48 /250`, with **no space after the stroke** (the stroke
  binds to the count it introduces), the total quieter again than the number it follows.
- **Both readouts sit in value wells** — the inset face of §2.1, which is what says
...
- **A layer number column** sits between the twirl and the label dot, in muted mono; the
  ...
  dot · name.
- **One gap, everywhere in an outline row: 8.** The mockup draws a layer's row as a
...
- **A row switch is a bare glyph**: no boxed or outlined face on the eye, solo,
  ...
  motion-blur switch.
- **A key's mark says its interpolation, half by half**: diamond linear, hourglass
  ...
  **11px** in Layers mode and in Keys; a shut layer's summary diamonds stay small.
- **The matte, blend and parent columns start at their content's width**: the
  mockup's 84 / 84 / 64 dropdown faces, and nothing beside them. Each seam still drags wider
  and stays where it is put. **The faces themselves are those widths** — a dropdown
  never swells into room it has not been given, which would put a third width in a row that
  draws two.
- **The matte column carries its two mode toggles' room only while a matte is set**:
...
- **The layer search sits in a 16px well** — the mockup's own height, with ground above and
  below it in the 19px row; it had sized itself to its glyph and filled the row.
...
- **The switches column is six cells**: visibility · audio · solo ·
...
- **The switches and Modes columns are fixed at their minimum width** — their toggles
  ...
  and neither seam is a drag handle (Modes added 2026-08-24 on the owner's word).
...
- **Two of the Modes cells are drawn by layer kind, and blank on the rows that cannot use
  them.** The flow-or-collapse cell is the first. The fifth is the **adjustment
  toggle**: the set's Adjustment mark, lit `text_primary` when the layer is an
...
- **The ruler is double height and reads as one band**: the times and the
  ...
  **work-area highlight sits on the ruler's second row**, and its **two drag handles run
  ...
  the band, never a second hex and never an alpha: a tab drawn through takes its
  ...
  *grabs* is unchanged in all three: the ruler's whole height either side of the edge is still the edge's
  ...
  resting zoom, three half-second ticks between labels two seconds apart.
...
- **The panel's bottom bar carries a toggle for the switches / modes / parent columns**, so
  the outline pares down to names and bars when the columns are not in use.
  **Each toggle hides the columns its own word names**: the parent picker is a
  ...
  and as words under `Chrome labels ▸ Words` (§5.1's setting consuming for the
...
### 12A.1a Keys mode — the dope sheet — **withdrawn**

> **Frozen.** Keys mode was withdrawn after desktop testing and its surfaces are deleted;
> nothing below describes anything the editor draws. It is kept because the withdrawal
> records *why* it went and what was declined with it, and this is the drawing it went by.
> The editing strip is the one part that survives, on the Layers bottom bar.
...
- **The outline's columns stand down** — and with them the bottom bar's toggles for those
  ...
  the sheet's filters: **Show — All / Animated** (Animated is the default; it is the
  redesign's filter, and `U` is the same reveal from the keyboard). **There is no scope
  pair**: the sheet is always every layer in the composition. It carried *Layers / Selected
  only* until the owner ruled the pair out — which properties are in hand is already said by
  the outline and by the wash on a picked lane.
- **A layer's row, then its properties flat.** The layer's own row carries its twirl, its
  **layer number** (in the outline's muted mono), its label-colour bullet, its name and —
  quietly, at the right — how many properties are listed under it. Each property is one
  row that **matches a Layers fold row** (superseding this bullet's earlier
  ...
  with every listed layer twirled open** — its own twirl default, not Layers
  mode's.
...
- **A key's shape is its interpolation** (§6.2's rule, brought out of the graph): diamond
  linear, square hold, **hourglass bezier** — the same marks as Layers, split at the
  vertical centre so each half answers for its own side. (The Keys drawing draws the bezier
  key round; the hourglass supersedes that, and the same painter draws both modes.) A key
  measures 11px point to point here and on a Layers lane alike.
- **The mode carries its drawing's editing tools** (superseding the earlier
  ...
  so Layers gains them wherever §12A.1 already names them as Layers behaviours.
- **The block box.** Two or more selected keys are one block. The box stands 4px inside
  ...
  with a 3×6 mark at each end inside an 11px hit target and the badge on
...
- The outline is **the Layers outline, identical** (superseding this entry's
...
- **Normalise is gone**: the owner finds it opaque. Every curve was scaled to its
...
- **Units are plain mono, never caps** (§7.1): mono at 10, muted, no tracking, in the gap
  ...
  centre is px@comp; now every centre is px@comp, which is the id-keyed table's other
...
- **Path reads from its left edge; every other column from its right.** Path is
...
- **Every column boundary is ruled, draggable or not**: the Timeline header's own
...
- **Per-type icon tints on media rows**, drawn from the layer-label palette, not
...
- **A third badge, `proxy`, is deliberately colourless**: it is drawn in
...
- **The bottom bar carries the project-wide proxies switch**, after the
...
- **The colour swatch filter lives inside that well, at its left** — one 10px
  square in the leading-mark slot a search glyph would take, with the well's own
...
- **Glyphs render at 13 throughout, rows and bottom bar alike** (§5): the 16 grid is
...
taking over from the boot splash and giving way to the shell as soon as somebody
...
- **The wordmark is the brand's own lockup at 22** — the website's
  ...
  It replaces the word set in mono at 0.08em, which was this section's earlier rule.
- **Two start cards, 63 tall and ten apart, sharing the column** — New project (save later)
  and Open (a `.lum`), and nothing else: the third card, which asked for a folder
...
- **The recents list is a kicker strip over a hairline well.** *Recent* is capitalised —
  ...
  the last (superseding the 40 written here before): the picture, then the name in
  body at 11 over the path in mono at 9 (`text_disabled`), then the date in mono at 10,
  muted, in a fixed 70 column.
- **Every row opens with a thumbnail**: the project as it looked when it was last
...
- **There is no format column** (superseding the 120px reserved here before). It was drawn
...
- **The footer is a 28px strip**: the version at the left in the sentence-case kicker face,
  ...
  library that printed it and belongs in the boot log and in bug reports.
- **New project is Save as, first**: the card opens the file picker immediately,
  ...
  starting work.
- **Escape closes the screen**, as it closes anything that has taken the window.
...
ships (the toolkit support is not stable yet; ordinary framework dialogs are the
...
- an optional tab row under it, **26** over its own hairline (Export: Output / Time /
  ...
  it follows the section last touched or scrolled to, and clicking a tab scrolls
...
- **label-left rows, the label in a fixed column and the control at the *start* of what is
  left**. **The column and the gap are the drawing's own**: Settings computes
  190 and 12 in rows of 30, New composition 110 and 12 in rows of 30, Export 100
  and 10 in rows of 28. Each is pinned by its own metrics test. **Recovery has no rows at
  all**: one sentence and three buttons, so its only measurement is its width;
- **kicker-titled groups, drawn as that dialog's drawing draws them**: a settings
...
- a footer strip of **45** — 10 above a 24px button and 10 below, over a hairline — carrying
  ...
  apart, 10 below: 108 for three, over the hairline;
- **buttons are sized by their content**, not paired to one width: 12px either side of an
  outlined label, 16 either side of the filled one, both 24 tall. Both approved drawings
  draw a pair that way.

**The Export dialog is one page, and every row on it is backed.** Colour and Metadata
stayed off the tab row while nothing could honour their rows, because an empty page is a
promise the dialog cannot keep; the engine half and the interface half have since landed.
Audio-only output, colour depth, channels and alpha, the output colour
...
**A whole section a format cannot use is dead all the way through**: a sound file
...
next to a note that said they did not matter. **The section itself states it**: the
...
answers rather than three.** *Motion blur* offers **Current settings**, **On for checked
...
**The Export dialog's preset controls are a strip of their own.** A preset sets and
...
**The right column of a paired row extends left into its own label.** The drawing
...
does was dropped with the drawing that had no room for it — a row may still carry a
...
### 12A.5 Feedback is transient and local
...
split allowed earlier is settled by the approved drawing: a **header strip** of 22
...
**Two marks the drawing does not draw at rest.** The **channel picker's closed
...
**The reading gives way in this order** (refining §12A.6's ladder below for one
compound line). The ladder's step 1 says "flexible text ellipsises", which for a line of four
...
**Two densities, and Regular is the default.** Regular is what the mockups actually
...
| Viewer bottom bar | 22 | 22 |
| Viewer bar glyphs — the marks, the transport, the view menu | 14 | 14 |
| Secondary rows elsewhere: filter rows, panel bottom bars, the graph's key readout | 19 | 18 |
| Timeline chrome row 1 — timecode, search, the Layers/Keys/Graph tabs | 24 | 18 |
...
| Timeline ruler — derived: chrome row 1 + chrome row 2 | 47 | 36 |
...
| Project panel: glyphs — the twirl and type marks in a row | 13 | 13 |
| Project panel: glyphs — the bottom bar's new-item controls | 13 | 13 |
...
| Welcome: a recent project's row (a seam under all but the last) | 52 | 52 |
| Welcome: a recent row's thumbnail — 16:9, 8 of air either side, 12 after it | 64×36 | 64×36 |
| Welcome: the recents' fixed columns — date, forget (no format column) | 70 / 12 | 70 / 12 |
...
| Export: the preset strip, at rest | 38 | 38 |
| Recovery: the window itself, and its stacked footer | 350 wide, footer 108 | 350 wide, footer 108 |
| Dialog value wells and dropdowns | 22 | 22 |
| Settings: the window itself | 760×520 | 760×520 |
...
ruler grows with it, which is exactly what the chrome-row ruling below did. The time
navigator stands above the derivation rather than inside it: the lane side spends the
strip's band above its ruler, and the outline spends the same band growing its timecode row
to the panel top (the owner's ruling), so the halves still meet and no dead ground stands
beside the strip.

**The Timeline's chrome rows are the owner's, not the manifests'.** Every other
...
point to point** (the mockup's 4px square has a 1px border and stands on its
corner, so it renders 8; the earlier reading of ≈5.7 measured the square's side and drew 5.
Against the **11px** mark a property's own lane draws in both Layers and Keys), a labelled ruler tick is **7px**
...
**The minimum is declared once, in one place, and enforced twice.** The floors are
...
**The Viewer's bottom bar sheds in this order, and the transport is last** (the
...
- A **filled socket is wired, a hollow one is not**; a dragged wire is dashed. A
  **bypassed node draws its border dashed** and its name goes muted; the **selected
  node's border is `animated`**.
- A **node header is an enable tick, a twirl and the name**, in that order and left to
  right — the Effect controls heading's own grammar, on a card. The tick is
  literally that heading's switch (`fxEnableMark`, the house checkbox at the heading's
...
- The canvas ground is `surface_0` under a dot grid one step lighter, whose dots stay
  ...
  canvas shrinks instead of multiplying. Nodes are
  `surface_1` cards with `surface_2` header strips and kicker-cased names — panel
  grammar, not a foreign look. The one exception is the **Custom shader** box:
...
## Brand: the mark and the splash

**The mark: the twin keyframes** (replacing the Kiriko facet placeholder). Two
...
study of ~1,270 top-chart and editing-app icons: at most two hue families, one large
glyph, dark tile, and none of the category's burned imagery (play button, film strip,
clapperboard, lens ring, colour wheel, AI sparkle).

Files, all in `assets/brand/`; the raster forms are regenerated by
`scripts/gen-icons.py`, except the macOS application icon, which Xcode compiles:
...
| `lumit-icon.icon` | **The macOS application icon**: the same composition as six Icon Composer layers, so macOS 26 lights it as Liquid Glass and renders the dark/tinted/clear variants. Xcode generates the pre-26 flat `.icns` from the same layers |
...
the `t` *are* the two keys. That file is the one the application ships too: copied
...
**The macOS layers carry no lighting of their own.** The layer SVGs inside
...
- **Text rendering at 11px.** Text polish is a known risk of the stack; if 11px mono kickers
...
- **`animated`'s closed list.** §3.1 closes the token's jobs at seven, but two of them — the
